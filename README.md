...
Inclusion in a block confirms the **guaranteed phase** passed but says nothing about whether the fallible phase (contract calls, verifier-key updates) actually succeeded. `TxInBlock::verdict` answers that, from the `TxApplied` / `TxPartialSuccess` events the Midnight pallet emits, so a completed `wait_best` / `wait_finalized` already carries it and no indexer is involved. The events name the transaction but not which segment failed; for a transaction with more than one fallible segment, `provider.get_transactions(TransactionOffset::hash(in_block.transaction_hash_hex()))` reads the indexer's per-segment breakdown. See [`docs/midnight-js-comparison.md`](docs/midnight-js-comparison.md) for the guaranteed/fallible phase model.
