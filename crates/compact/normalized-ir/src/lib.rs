//! Reader for `compiler/normalized-ir.sexp`, the Compact compiler's analyzed
...
/// Parse one `normalized-ir.sexp` artifact from its text.
pub fn parse_str(src: &str) -> Result<NormalizedIr, Error> {
