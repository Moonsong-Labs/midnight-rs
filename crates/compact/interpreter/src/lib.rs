...
use midnight_helpers::onchain_runtime::context::QueryContext;
use midnight_helpers::onchain_runtime::cost_model::INITIAL_COST_MODEL;
use midnight_helpers::onchain_runtime::ops::{Key, Op};
use midnight_helpers::onchain_runtime::result_mode::{GatherEvent, ResultModeGather};
...
pub fn execute_with_owned(
    ...
    contract_address: Option<midnight_helpers::coin_structure::contract::ContractAddress>,
) -> Result<ExecutionResult, InterpreterError> {
...
fn default_value(ty: &Type) -> Result<Value, InterpreterError> {
    ...
    match ty {
        ...
        Type::Point(_) => Ok(Value::AlignedValue(AlignedValue::from(
            midnight_helpers::transient_crypto::curve::EmbeddedGroupAffine::identity(),
        ))),
...
fn exec_ledger_query(
    ...
    if std::env::var("INTERPRETER_DEBUG").is_ok() {
        ...
        if let Some(midnight_helpers::onchain_runtime::ops::Op::Idx { path, .. }) = ops.get(1) {
...
fn max_sizeof(ty: &Type) -> Result<u64, InterpreterError> {
    use midnight_helpers::transient_crypto::fab::AlignmentExt;
...
mod tests {
    ...
    fn ec_mul_generator_matches_direct_call() {
        use midnight_helpers::transient_crypto::curve::{EmbeddedGroupAffine, Fr};
    ...
    fn fr_two_pow_64() -> midnight_helpers::transient_crypto::curve::Fr {
        use midnight_helpers::transient_crypto::curve::Fr;
