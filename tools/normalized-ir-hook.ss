;;; normalized-ir-hook.ss: print the analyzed IR (Lloweredemit) as it is,
;;; for use with compactc --run-hook. Writes compiler/normalized-ir.sexp.
;;;
...
;;;
;;; Usage: compactc --skip-zk --run-hook normalized-ir-hook.ss <src> <target-dir>

(import (except (chezscheme) errorf)
        (langs) (nanopass) (vm) (utils) (pass-helpers)
        (compiler-version) (language-version) (runtime-version))

(define (fail what x) (external-errorf "normalized-ir-hook: unsupported ~a: ~s" what x))
...
(define hook
  (lambda (pass-name unparse formats . x*)
    ...
    (when (eq? pass-name 'save-contract-info)
      (with-output-to-file (format "~a/compiler/normalized-ir.sexp" (target-directory))
        (lambda ()
