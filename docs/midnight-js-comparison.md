...
Private state is also **threaded** through witness execution, matching midnight-js. `WitnessProvider::call_witness(ctx, name, args)` takes a `&mut WitnessContext` carrying the mutable private state (the analogue of midnight-js's `(ctx, ...args) => [newPS, result]`). When a `PrivateStateProvider` is attached, a circuit call loads the contract's state before execution, threads it through the witnesses, and persists the updated state after the tx lands — so stateful-witness contracts work across calls without the caller managing storage. Threading uses the private-state id set on the call chain (`circuits().with_private_state_id(..)`, default `"default"`), so a contract can hold several private states under different ids.
