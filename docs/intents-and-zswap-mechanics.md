...
That's correct on any chain whose successive blocks track real time. It's a trap on the **local dev devnet**, where block 0 ships with a hardcoded `tblock` from months before wall clock (the dev preset's genesis is stamped months earlier) but block 1+ uses the validator's real clock. If you build a transaction while the chain has only genesis, `intent.ttl` lands months in the past relative to the chain's view as soon as block 1 arrives — submission is rejected with chain custom error 228.

The SDK does not guard against this. It anchors on the latest block the indexer reports, and while the chain holds only genesis that block *is* genesis. Let the chain produce a block before you build. `make dev-up` waits for one; a devnet you start yourself needs the same wait, for example by polling the indexer's `block { height }` until it reaches 1. Any chain past block 1 (mainnet, preprod, or a local devnet older than one block) anchors on real time and needs nothing.
